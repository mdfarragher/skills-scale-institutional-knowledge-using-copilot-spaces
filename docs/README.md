...
OctoAcme projects are staffed by clearly defined personas: the **Project Manager (PM)** coordinates delivery, schedules, risk, and cross-team communications; the **Product Manager (PdM)** owns the product vision, backlog prioritization, and outcome measurement; the **Technical Lead** owns technical decisions, architecture, and engineering standards; **Developers** implement features, maintain test coverage, and participate in design and code review; the **QA Lead / Quality Owner** defines the test strategy, owns quality gates, and leads all QA/Testing activities; the **UX Designer** owns user research, interaction design, and usability validation; the **Support / Customer Advocate** channels customer feedback into the backlog and incident reviews; the **Release Manager** coordinates release planning, the deployment checklist, and go/no-go decisions (may be played by the PM on small teams); and **Stakeholders** provide direction, inputs, and approvals at key decision gates.
...
| [Role Assignment Checklist](octoacme-role-assignment-checklist.md) | Quick-start checklist for assigning roles and confirming accountabilities on a new project |
